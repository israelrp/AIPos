...
namespace AIPos.DAO.Implementation
{
    public class PedidoSucursalDaoImpl : PedidoSucursalDao
    {
        ...
        public List<ReportePedidoSucursal> SelectReporteBySucursalFechaEntrega(int SucursalId, DateTime FechaInicio, DateTime FechaFin)
        {
            object[] parameters = new object[] { SucursalId, FechaInicio, FechaFin };
            return cm.Database.SqlQuery<ReportePedidoSucursal>("dbo.usp_PedidosSucursalReporteSelectBySucursalFechaEntrega @SucursalId={0}, @FechaInicio={1}, @FechaFin={2}", parameters).ToList();
        }
